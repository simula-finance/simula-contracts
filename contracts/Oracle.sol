...
contract Oracle is Ownable {
    ...
    function twap(address token, uint256 pricePrecision)
        external
        view
    ...
    function spot(address token, uint256 pricePrecision)
        external
        view
        returns (uint256 amountOut)
    {
        ...
        if (token == _token0) {
            _price = _reserve1
                .mul(10**_token1MissingDecimals)
                .mul(pricePrecision)
                .div(_reserve0);
        } else {
            _price = _reserve0
                .mul(10**_token0MissingDecimals)
                .mul(pricePrecision)
                .div(_reserve1);
        }
        return _price;
